...
HRESULT InitGeometry()
{
    ...
    if (FAILED(g_pVB->Lock(0, 0, (void**)&pVertices, 0)))
        return E_FAIL;
    pVertices[1].position = D3DXVECTOR3(-1.0f, 1.0f, 0.0f);//左上
    pVertices[1].color = 0xffffffff;
    pVertices[1].tu = 0.0f;
    pVertices[1].tv = 0.0f;

    pVertices[0].position = D3DXVECTOR3(-1.0f, -1.0f, 0.0f); //左下
    pVertices[0].color = 0xffffffff;
    pVertices[0].tu = 0.0f;
    pVertices[0].tv = 1.0f;
              
    pVertices[2].position = D3DXVECTOR3(1.0f, -1.0f, 0.0f);//右下
    pVertices[2].color = 0xffffffff;
    pVertices[2].tu = 1.0f;
    pVertices[2].tv = 1.0f;

    pVertices[3].position = D3DXVECTOR3(1.0f, 1.0f, 0.0f);//右上
...
VOID SetupMatrices()
{
    ...
    D3DXMatrixPerspectiveFovLH( &matProj, D3DX_PI / 8, 1.0f, 1.0f, 100.0f );
...
INT WINAPI wWinMain( HINSTANCE hInst, HINSTANCE, LPWSTR, INT )
{
    ...
    HWND hWnd = CreateWindow( L"D3D Tutorial", L"D3D Tutorial 05: Textures",
                              WS_OVERLAPPEDWINDOW, 100, 100, 392, 190,
